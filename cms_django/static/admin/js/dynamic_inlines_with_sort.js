...
position_field = 'sort_order'; // Name of inline model field (integer) used for ordering. Defaults to "position".
...
django.jQuery(function($) {    
    ...
    $('div.inline-group div.tabular').each(function() {
        ...
        if (position_field != '' && table.find('td').is('.' + position_field))
        {
            ...
            $(this).find('td.' + position_field).css('visibility', 'hidden');
            td_pos_field_index = table.find('tbody tr td').index($(this).find('td.' + position_field));
            $(this).find('th:eq(' + (td_pos_field_index-1) + ')').text('Block Name');
